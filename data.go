...
type Options struct {
	...
	// Allow overwrite Cache-Control header value with max-age ttl in sec
	TTL int
...
//	ttl3600 - set Cache-Control header in response with max-age:3600(one hour)
func ParseOptions(str string) Options {
	...
	for _, opt := range strings.Split(str, ",") {
		...
		case strings.HasPrefix(opt, optTTL):
			value := strings.TrimPrefix(opt, optTTL)
			options.TTL, _ = strconv.Atoi(value)
